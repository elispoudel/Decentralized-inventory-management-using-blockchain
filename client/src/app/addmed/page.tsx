...
export default function AddMed() {
  ...
  const [medOrderedAt, setMedOrderedAt] = useState<{ [key: number]: string }>({})
  ...
  const loadBlockchainData = async () => {
    try {
      ...
      for (let i = 0; i < medCtr; i++) {
        const chainId = i + 1
        medData[i] = await contract.methods.MedicineStock(chainId).call()
        medStageData[i] = await contract.methods.showStage(chainId).call()
        orderedAtData[i] = await contract.methods.medicineCreatedAt(chainId).call()
      }

      setMed(medData)
      setMedStage(medStageData)
      setMedOrderedAt(orderedAtData)
                  ...
                  {Object.keys(med).map((key) => {
                    ...
                    return (
                      <tr key={key} className="hover:bg-green-50 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center">
                            <div className="w-10 h-10 bg-gradient-to-br from-green-500 to-emerald-500 rounded-lg flex items-center justify-center text-white font-bold mr-3 shadow-md text-xs">
                              {formatOrderCode(med[index].id)}
                            </div>
                            <div>
                              <span className="font-semibold text-gray-800">{formatOrderCode(med[index].id)}</span>
                              <div className="text-xs text-gray-500">Internal #{med[index].id}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 text-gray-600 text-sm">{formatUnixTimestamp(medOrderedAt[index])}</td>
                        <td className="px-6 py-4">
