...
import { dashboardHomePath } from '@/lib/auth/clientSession'
...
export default function AddMed() {
  ...
  const [productOrderedAt, setProductOrderedAt] = useState<{ [key: number]: string }>({})
  ...
  const loadBlockchainData = async () => {
    try {
      ...
      const productStageData: string[] = []
      const orderedAtData: { [key: number]: string } = {}

      for (let i = 0; i < medCtr; i++) {
        const chainId = i + 1
        productData[i] = await contract.methods.ProductStock(chainId).call()
        productStageData[i] = await contract.methods.showStage(chainId).call()
        orderedAtData[i] = await contract.methods.productCreatedAt(chainId).call()
      }

      setProduct(productData)
      setProductStage(productStageData)
      setProductOrderedAt(orderedAtData)
  ...
  const handlerChangeNameProduct = (event: React.ChangeEvent<HTMLInputElement>) => {
    setProductName(event.target.value)
  }

  const handlerChangeDescriptionProduct = (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    setProductDes(event.target.value)
  }

  const handlerSubmitProduct = async (event: React.FormEvent) => {
    event.preventDefault()
    setIsSubmitting(true)
    try {
      const receipt = await supplyChain.methods.addProduct(productName, productDes).send({ from: currentAccount })
      if (receipt) {
        loadBlockchainData()
        setProductName('')
        setProductDes('')
      ...
      if (errorMessage.includes('revert') || errorMessage.includes('require')) {
        if (errorMessage.includes('Owner')) {
          errorMessage = 'Only the contract owner can add products. Make sure you are using the account that deployed the contract.'
        } else if (errorMessage.includes('manCtr') || errorMessage.includes('warCtr') || errorMessage.includes('retCtr')) {
          errorMessage = 'Please register at least one role of each type (Manufacturer, Warehouse, Retailer) before adding products.'
        } else {
  ...
  const formatOrderCode = (idValue: string | number) => {
    ...
    if (!Number.isFinite(numericId)) return 'B00000'
    return `B${numericId.toString().padStart(5, '0')}`
  }
  ...
  const getStageColor = (stage: string) => {
    ...
    if (stage.includes('Warehouse')) return 'bg-purple-100 text-purple-700 border-purple-300'
  ...
  return (
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 p-5">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600 text-sm">Create new product orders in the supply chain</p>
              </div>
            </div>
            <button 
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3 flex-1">
                ...
                <p className="text-red-700 text-sm mb-2">
                  Only the contract owner can create product orders.
                </p>
        ...
        {isOwner && (roleCounts.man === 0 || roleCounts.war === 0 || roleCounts.ret === 0) && (
          <div className="mb-6 p-5 bg-yellow-50 border-l-4 border-yellow-500 rounded-xl shadow-lg">
            <div className="flex items-start mb-4">
              ...
              <div className="ml-3 flex-1">
                ...
                <p className="text-yellow-700 text-sm">
                  You must register at least one role of each type before ordering products.
                </p>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-4">
              ...
              <div className={`p-3 rounded-lg border-2 ${roleCounts.war > 0 ? 'bg-green-50 border-green-300' : 'bg-red-50 border-red-300'}`}>
                <div className="flex items-center mb-2">
                  <svg className={`w-5 h-5 mr-2 ${roleCounts.war > 0 ? 'text-green-600' : 'text-red-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    {roleCounts.war > 0 ? (
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    ) : (
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  ...
                  <div className="text-xs font-semibold">Warehouse</div>
                </div>
                <div className={`text-lg font-bold ${roleCounts.war > 0 ? 'text-green-700' : 'text-red-700'}`}>
                  {roleCounts.war} registered
                </div>
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-6">
          <div className="flex items-center mb-6">
            ...
            <h2 className="text-2xl font-bold text-gray-800">Create New Product Order</h2>
          </div>
          
          <form onSubmit={handlerSubmitProduct} className="space-y-5">
            <div className="relative">
              ...
              <input
                ...
                type="text"
                onChange={handlerChangeNameProduct}
            ...
            <div className="relative">
              ...
              <textarea
                className="w-full pl-12 pr-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all text-lg min-h-[160px] resize-y"
                rows={6}
                onChange={handlerChangeDescriptionProduct}
                placeholder="Product Description (press Enter for a new line)"
                value={productDes}
                required
                disabled={isSubmitting}
              />
            ...
            <button
              type="submit"
              disabled={!isOwner || roleCounts.man === 0 || roleCounts.war === 0 || roleCounts.ret === 0 || isSubmitting}
              className={`w-full px-6 py-4 rounded-xl transition-all font-semibold text-lg flex items-center justify-center shadow-lg ${
                isOwner && roleCounts.man > 0 && roleCounts.war > 0 && roleCounts.ret > 0 && !isSubmitting
            ...
            >
              ...
              ) : roleCounts.man === 0 || roleCounts.war === 0 || roleCounts.ret === 0 ? (
                <>
                  <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                  </svg>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center">
              ...
              <h2 className="text-2xl font-bold text-gray-800">Ordered Products</h2>
            </div>
          ...
          {Object.keys(product).length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 text-lg font-semibold">No products ordered yet</p>
                <p className="text-gray-400 text-sm mt-2">Create your first product order above</p>
            </div>
          ) : (
              ...
              <table className="min-w-full">
                <thead>
                  ...
                  {Object.keys(product).map((key) => {
                    ...
                    const stage = productStage[index]
                    return (
                      <tr key={key} className="hover:bg-green-50 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center">
                            <div className="w-10 h-10 bg-gradient-to-br from-green-500 to-emerald-500 rounded-lg flex items-center justify-center text-white font-bold mr-3 shadow-md text-xs">
                              {formatOrderCode(product[index].id)}
                            </div>
                            <div>
                              <span className="font-semibold text-gray-800">{formatOrderCode(product[index].id)}</span>
                              <div className="text-xs text-gray-500">Internal #{product[index].id}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 font-medium text-gray-800">{product[index].name}</td>
                        <td className="px-6 py-4 text-gray-600">{product[index].description}</td>
                        <td className="px-6 py-4 text-gray-600 text-sm">{formatUnixTimestamp(productOrderedAt[index])}</td>
                        <td className="px-6 py-4">
