...
function emailForChainAddr(roleType: 'man' | 'war' | 'ret', addr: string, db: DbRegistrations): string {
  const key = roleType === 'man' ? 'manufacturer' : roleType === 'war' ? 'warehouse' : 'retailer'
  const list = db[key]
  const a = addr.toLowerCase()
  const row = list.find((r) => r.ethereumAddress && r.ethereumAddress.toLowerCase() === a)
  return row?.email ?? '—'
}
...
export default function AssignRoles() {
  ...
  const loadBlockchainData = async () => {
    try {
      ...
      setRoles({ man, war, ret })

      await loadDbRegistrations()
  ...
  const loadDbRegistrations = async () => {
  ...
  const handleRoleSubmit = async (event: React.FormEvent) => {
    ...
    const { address, name, place, type, email, password } = newRole
    const session = loadSession()
    if (!session?.token) {
    ...
    const roleApi: Record<'man' | 'war' | 'ret', 'manufacturer' | 'warehouse' | 'retailer'> = {
      man: 'manufacturer',
      war: 'warehouse',
      ret: 'retailer',
    }
    ...
    try {
      const res = await fetch('/api/admin/register-role', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          email: email.trim(),
          password,
          role: roleApi[type as 'man' | 'war' | 'ret'],
          location: place,
      ...
      if (!res.ok) {
        alert(data.error || 'Could not create account. Check email and fields.')
        return
      }
      ...
      switch (type) {
        case 'man':
          ...
          break
        case 'war':
          receipt = await supplyChain.methods.addWarehouse(address, name, place).send({ from: currentAccount })
          break
        case 'ret':
        ...
        default:
          ...
          if (userId) {
            await fetch('/api/admin/register-role', {
              method: 'DELETE',
              headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${session.token}`,
      ...
      if (receipt) {
        ...
        setNewRole({ address: '', name: '', place: '', type: 'man', email: '', password: '' })
      }
    } catch (err: any) {
      if (userId) {
        try {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-5">
      <div className="max-w-6xl mx-auto">
        ...
        <div className={`bg-gradient-to-br ${roleConfig[newRole.type as keyof typeof roleConfig].bgGradient} rounded-2xl shadow-xl p-8 mb-6 border-l-4 ${roleConfig[newRole.type as keyof typeof roleConfig].borderColor}`}>
          ...
          <form onSubmit={handleRoleSubmit} className="space-y-4">
            <div>
              ...
              <input
                className="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent text-lg bg-white"
                type="text"
                name="name"
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Ethereum Address</label>
              <input
                className="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent text-lg bg-white font-mono text-sm"
                type="text"
                name="address"
                placeholder="0x..."
                onChange={handleInputChange}
        ...
        <div className="space-y-6">
          ...
          {(['man', 'war', 'ret'] as const).map((roleType) => {
                    ...
                    <table className="min-w-full">
                      <thead>
                        <tr className={`bg-gradient-to-r ${config.bgGradient}`}>
                          ...
                          <th className="px-6 py-4 text-left text-sm font-bold text-gray-700">Email Id</th>
                        ...
                        {roleList.map((role, index) => (
                          <tr key={index} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-6 py-4 text-gray-700 text-sm">
                              {emailForChainAddr(roleType, role.addr, dbRegistrations)}
                            </td>
