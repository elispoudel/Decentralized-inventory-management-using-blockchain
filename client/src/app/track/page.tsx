  ...
  const loadBlockchainData = async () => {
    try {
      ...
      for (let i = 0; i < medCtr; i++) {
        const medicineId = i + 1
        medData[medicineId] = await contract.methods.MedicineStock(medicineId).call()
        medStageData[medicineId] = await contract.methods.showStage(medicineId).call()
        medTimestampData[medicineId] = await contract.methods.getMedicineTimestamps(medicineId).call()
      }
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-purple-100 text-sm">Track Code: {formatOrderCode(med[medicineId]?.id || 0)}</p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <div className="rounded-lg bg-blue-50 p-3">
              ...
              <div className="text-xs text-gray-700 mt-1">{formatUnixTimestamp(medTimestamps[medicineId]?.orderedAt)}</div>
            </div>
            <div className="rounded-lg bg-green-50 p-3">
              <div className="text-xs text-green-700">Raw Supply</div>
              <div className="text-xs text-gray-700 mt-1">{formatUnixTimestamp(medTimestamps[medicineId]?.rawSupplyAt)}</div>
            </div>
            <div className="rounded-lg bg-yellow-50 p-3">
              <div className="text-xs text-yellow-700">Manufacture</div>
              <div className="text-xs text-gray-700 mt-1">{formatUnixTimestamp(medTimestamps[medicineId]?.manufactureAt)}</div>
            </div>
            <div className="rounded-lg bg-purple-50 p-3">
              <div className="text-xs text-purple-700">Distribution</div>
              <div className="text-xs text-gray-700 mt-1">{formatUnixTimestamp(medTimestamps[medicineId]?.distributionAt)}</div>
            </div>
            <div className="rounded-lg bg-orange-50 p-3">
              <div className="text-xs text-orange-700">Retail</div>
              <div className="text-xs text-gray-700 mt-1">{formatUnixTimestamp(medTimestamps[medicineId]?.retailAt)}</div>
            </div>
            <div className="rounded-lg bg-gray-100 p-3">
              <div className="text-xs text-gray-700">Sold</div>
              <div className="text-xs text-gray-700 mt-1">{formatUnixTimestamp(medTimestamps[medicineId]?.soldAt)}</div>
            </div>
          </div>
        </div>
                  ...
                  {Object.keys(med).map((key) => {
                    ...
                    return (
                      <tr key={key} className="hover:bg-purple-50 transition-colors cursor-pointer" onClick={() => trackMedicine(medicineId)}>
                        <td className="px-6 py-4">
                          <div className="flex items-center">
                            <div className="w-10 h-10 bg-gradient-to-br from-purple-500 to-pink-500 rounded-lg flex items-center justify-center text-white font-bold mr-3 text-xs">
                              {formatOrderCode(med[medicineId].id)}
                            </div>
                            <div>
                              <span className="font-semibold text-gray-800">{formatOrderCode(med[medicineId].id)}</span>
                              <div className="text-xs text-gray-500">Internal #{med[medicineId].id}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 text-gray-600 text-sm">{formatUnixTimestamp(medTimestamps[medicineId]?.orderedAt)}</td>
                        <td className="px-6 py-4">
