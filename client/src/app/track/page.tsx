...
import { getStageLabel, parseStageValue } from '@/lib/stageUtils'
...
export default function Track() {
  ...
  const [trackTillWarehouse, setTrackTillWarehouse] = useState(false)
  ...
  const loadBlockchainData = async (opts?: { silent?: boolean }) => {
    const silent = opts?.silent === true
    try {
      if (!silent) setLoader(true)
      ...
      for (let i = 0; i < medCtr; i++) {
        const productId = i + 1
        productData[productId] = await contract.methods.ProductStock(productId).call()
        const sn = parseStageValue(productData[productId].stage)
        productStageData[productId] =
          sn >= 0 ? getStageLabel(sn) : ((await contract.methods.showStage(productId).call()) as string)
        productTimestampData[productId] = await contract.methods.getProductTimestamps(productId).call()
      }
  ...
  const trackProduct = async (productId: number) => {
    try {
      setTrackTillSold(false)
      setTrackTillRetail(false)
      setTrackTillWarehouse(false)
      setTrackTillManufacture(false)
      setTrackTillOrdered(false)

      const { contract } = await getContract()
      const ctr = await contract.methods.productCtr().call()
      if (!(productId > 0 && productId <= parseInt(ctr))) {
        alert('Invalid Product ID!!!')
        return
      }

      const fresh = await contract.methods.ProductStock(productId).call()
      const stage = parseStageValue(fresh.stage)
      const stageLabel =
        stage >= 0 ? getStageLabel(stage) : ((await contract.methods.showStage(productId).call()) as string)
      const ts = await contract.methods.getProductTimestamps(productId).call()

      setId(productId.toString())
      setProduct((prev) => ({ ...prev, [productId]: fresh }))
      setProductStage((prev) => ({ ...prev, [productId]: stageLabel }))
      setProductTimestamps((prev) => ({ ...prev, [productId]: ts }))

      if (stage === 4) setTrackTillSold(true)
      else if (stage === 3) setTrackTillRetail(true)
      else if (stage === 2) setTrackTillWarehouse(true)
      else if (stage === 1) setTrackTillManufacture(true)
  ...
  const resetTracking = () => {
    ...
    const formatOrderCode = (idValue: string | number) => {
    const numericId = Number(idValue)
    if (!Number.isFinite(numericId)) return 'P00000'
    return `P${numericId.toString().padStart(5, '0')}`
  }
  ...
  const TrackComponent = ({
    ...
    stages: Array<{
      ...
      showArrow?: boolean
      icon?: JSX.Element
      /** Final step with no participant card (e.g. sold) */
      terminal?: boolean
    }>
  }) => {
    const productId = parseInt(id)
    const stageNum = parseStageValue(product[productId]?.stage)
    const currentStageText =
      stageNum >= 0 ? getStageLabel(stageNum) : productStage[productId] || '—'
    const productData = {
        ...
        <div className="bg-gradient-to-br from-purple-500 to-pink-600 rounded-2xl shadow-2xl p-8 mb-8 text-white">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-purple-100 text-sm">Track Code: {formatOrderCode(product[productId]?.id || 0)}</p>
              </div>
            </div>
            <button
              onClick={async () => {
              ...
              className="px-4 py-2 bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-lg text-sm font-semibold hover:from-cyan-600 hover:to-blue-600"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
              ...
              <div className="text-lg font-semibold">{product[productId]?.name}</div>
            </div>
            <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
              <div className="text-sm text-purple-100 mb-1">Description</div>
              <div className="text-lg font-semibold truncate">{product[productId]?.description}</div>
            </div>
            <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
              <div className="text-sm text-purple-100 mb-1">Current Stage</div>
              <div className="text-lg font-semibold">{currentStageText}</div>
            </div>
          </div>
        </div>

        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6">
          <h4 className="text-xl font-bold text-gray-800 mb-4">Stage Timestamps</h4>
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-3">
            <div className="rounded-lg bg-blue-50 p-3">
              <div className="text-xs text-blue-700">Ordered</div>
              <div className="text-xs text-gray-700 mt-1">{formatUnixTimestamp(productTimestamps[productId]?.orderedAt)}</div>
            </div>
            <div className="rounded-lg bg-green-50 p-3">
              <div className="text-xs text-green-700">Manufacturing</div>
              <div className="text-xs text-gray-700 mt-1">{formatUnixTimestamp(productTimestamps[productId]?.manufactureAt)}</div>
            </div>
            <div className="rounded-lg bg-yellow-50 p-3">
              <div className="text-xs text-yellow-700">Warehouse</div>
              <div className="text-xs text-gray-700 mt-1">{formatUnixTimestamp(productTimestamps[productId]?.warehouseAt)}</div>
            </div>
            <div className="rounded-lg bg-orange-50 p-3">
              <div className="text-xs text-orange-700">Retail</div>
              <div className="text-xs text-gray-700 mt-1">{formatUnixTimestamp(productTimestamps[productId]?.retailAt)}</div>
            </div>
            <div className="rounded-lg bg-gray-100 p-3">
              <div className="text-xs text-gray-700">Sold</div>
              <div className="text-xs text-gray-700 mt-1">{formatUnixTimestamp(productTimestamps[productId]?.soldAt)}</div>
            </div>
          </div>
        </div>
                  ...
                  <div className="ml-6 flex-1 bg-gray-50 rounded-xl p-6 shadow-md hover:shadow-lg transition-shadow">
                    ...
                    {stage.terminal ? (
                      <div className="bg-green-50 border-l-4 border-green-500 p-4 rounded">
                        <p className="text-green-900 font-medium">Sale finalized and recorded on the blockchain.</p>
                      </div>
                    ) : stage.data ? (
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          stages={[
            {
              ...
              data: man[parseInt(product[productId]?.MANid)],
              showArrow: true,
              icon: stageIcons.manufacture,
            },
            {
              label: 'Warehoused by',
              data: war[parseInt(product[productId]?.WARid)],
              showArrow: true,
              icon: stageIcons.warehouse,
            },
            {
              label: 'Retailed by',
              data: ret[parseInt(product[productId]?.RETid)],
              showArrow: true,
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-100 p-5">
        <TrackComponent
          ...
          stages={[
            {
              label: 'Product Not Yet Processed...',
              showArrow: false,
            },
          ]}
        />
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => loadBlockchainData({ silent: true })}
                className="px-4 py-2 bg-white border border-purple-200 text-purple-700 rounded-lg hover:bg-purple-50 transition-colors flex items-center text-sm font-semibold"
              >
                <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                </svg>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-6">
          <div className="flex items-center mb-6">
            ...
            <h2 className="text-2xl font-bold text-gray-800">Enter Product ID to Track</h2>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="text-sm text-gray-500">
              Total: {Object.keys(product).length} items
            </div>
          </div>
          ...
          {Object.keys(product).length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 text-lg">No batteries available yet</p>
                <p className="text-gray-400 text-sm mt-2">Add products in the Order Products page</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full">
                  ...
                  <tr className="bg-gradient-to-r from-purple-50 to-pink-50">
                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700">Batch Code</th>
                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700">Name</th>
                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700">Description</th>
                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700">Ordered At</th>
                  ...
                  {Object.keys(product).map((key) => {
                    ...
                    const sn = parseStageValue(product[productId]?.stage)
                    const stage =
                      sn >= 0 ? getStageLabel(sn) : productStage[productId] || '—'
                    const getStageColor = (label: string) => {
                      if (label.includes('Ordered')) return 'bg-blue-100 text-blue-700'
                      if (label.includes('Manufacturing')) return 'bg-yellow-100 text-yellow-700'
                      if (label.includes('Warehouse')) return 'bg-purple-100 text-purple-700'
                      if (label.includes('Retail')) return 'bg-orange-100 text-orange-700'
                      if (label.includes('Sold')) return 'bg-emerald-100 text-emerald-800'
                      return 'bg-gray-100 text-gray-700'
                    }
                    return (
                      <tr key={key} className="hover:bg-purple-50 transition-colors cursor-pointer" onClick={() => trackProduct(productId)}>
                        <td className="px-6 py-4">
                          <div className="flex items-center">
                            <div className="w-10 h-10 bg-gradient-to-br from-purple-500 to-pink-500 rounded-lg flex items-center justify-center text-white font-bold mr-3 text-xs">
                              {formatOrderCode(product[productId].id)}
                            </div>
                            <div>
                              <span className="font-semibold text-gray-800">{formatOrderCode(product[productId].id)}</span>
                              <div className="text-xs text-gray-500">Internal #{product[productId].id}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 font-medium text-gray-800">{product[productId].name}</td>
                        <td className="px-6 py-4 text-gray-600">{product[productId].description}</td>
                        <td className="px-6 py-4 text-gray-600 text-sm">{formatUnixTimestamp(productTimestamps[productId]?.orderedAt)}</td>
                        <td className="px-6 py-4">
