...
export default function Supply() {
  ...
  const [supplyViewerRole, setSupplyViewerRole] = useState<UserRole | null>(null)

  useEffect(() => {
    const s = loadSession()
    if (s?.user?.role) setSupplyViewerRole(s.user.role)
  }, [])
  ...
  const loadBlockchainData = async () => {
    try {
      ...
      for (let i = 0; i < medCtr; i++) {
        productData[i] = await contract.methods.ProductStock(i + 1).call()
        const sn = parseStageValue(productData[i].stage)
        productStageData[i] =
          sn >= 0 ? getStageLabel(sn) : ((await contract.methods.showStage(i + 1).call()) as string)
      }
  ...
  const handlerChangeWarId = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handlerSubmitManufacturing = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMessage.includes('revert') || errorMessage.includes('require')) {
        if (errorMessage.includes('findMAN')) {
          ...
          errorMessage = 'Invalid stage transition. Make sure the product is in the correct stage for this operation.'
        } else if (errorMessage.includes('productID') || errorMessage.includes('_productID')) {
          errorMessage = 'Invalid product ID. Please check the product ID and try again.'
        } else {
  ...
  const handlerSubmitWarehouse = async (event: React.FormEvent) => {
    event.preventDefault()
    try {
      const receipt = await supplyChain.methods.Warehouse(warId).send({ from: currentAccount })
      if (receipt) {
        loadBlockchainData()
        setWarId('')
      ...
      // Check for common revert reasons
      if (errorMessage.includes('revert') || errorMessage.includes('require')) {
        if (errorMessage.includes('findWAR') || errorMessage.includes('findRET')) {
          errorMessage = 'Your account is not registered for this role. Please register your account first in the Roles page.'
        } else if (errorMessage.includes('stage') || errorMessage.includes('STAGE')) {
          errorMessage = 'Invalid stage transition. Make sure the product is in the correct stage for this operation.'
        } else if (errorMessage.includes('productID') || errorMessage.includes('_productID')) {
  ...
  const handlerSubmitRetail = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (receipt) {
        loadBlockchainData()
        setRetId('')
        alert('Retail stage completed. Use Mark as sold below to record Product Sold.')
      }
    } catch (err: any) {
      let errorMessage = 'An error occurred!'
      if (err?.message) {
      ...
      // Check for common revert reasons
      if (errorMessage.includes('revert') || errorMessage.includes('require')) {
        if (errorMessage.includes('findMAN') || errorMessage.includes('findWAR') || errorMessage.includes('findRET')) {
          errorMessage = 'Your account is not registered for this role. Please register your account first in the Roles page.'
        } else if (errorMessage.includes('stage') || errorMessage.includes('STAGE')) {
          errorMessage = 'Invalid stage transition. Make sure the product is in the correct stage for this operation.'
        } else if (errorMessage.includes('productID') || errorMessage.includes('_productID')) {
  ...
  const handlerSubmitSold = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMessage.includes('revert') || errorMessage.includes('require')) {
        if (errorMessage.includes('findRET')) {
          errorMessage = 'Your account is not registered as a retailer. Register on the Roles page.'
        } else if (errorMessage.includes('Wrong retailer')) {
          errorMessage = 'Only the retailer who received the product in Step 3 can mark it sold.'
        } else if (errorMessage.includes('stage') || errorMessage.includes('STAGE')) {
          errorMessage = 'Product must be at Retail Stage before marking sold. Complete Step 3 first.'
        } else if (errorMessage.includes('productID') || errorMessage.includes('_productID')) {
  ...
  const r = supplyViewerRole
  const isAdminViewer = r === 'admin'
  const roleSplit = r === 'manufacturer' || r === 'warehouse' || r === 'retailer'
  const showFullFlowDiagram = isAdminViewer || !roleSplit
  const showManufacture = isAdminViewer || !roleSplit || r === 'manufacturer'
  const showWarehouse = isAdminViewer || !roleSplit || r === 'warehouse'
  const showRetailerSections = isAdminViewer || !roleSplit || r === 'retailer'
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600 text-sm">Manage the flow of products through the supply chain</p>
              </div>
            </div>
            <button
            ...
            {showFullFlowDiagram ? (
              <>
                ...
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-xs mt-2 text-gray-700 font-semibold text-center">Warehouse</span>
                </div>
                <svg className="w-6 h-6 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
              ...
              <div className="flex flex-col items-center py-2">
                ...
                <span className="text-sm mt-3 text-gray-800 font-bold text-center">Manufacture</span>
              </div>
            ) : r === 'warehouse' ? (
              <div className="flex flex-col items-center py-2">
                <div className="w-14 h-14 bg-purple-500 rounded-full flex items-center justify-center text-white shadow-lg">
                  <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
                  </svg>
                </div>
                <span className="text-sm mt-3 text-gray-800 font-bold text-center">Warehouse</span>
              </div>
            ) : (
              <>
                <div className="flex flex-col items-center">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6">
          ...
          {Object.keys(product).length === 0 ? (
            <div className="text-center py-12">
              ...
              <h1 className="text-3xl font-bold text-gray-800">No products available yet</h1>
                      ...
                      <tr key={key} className="hover:bg-orange-50 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center">
                            ...
                            <span className="font-semibold text-gray-800">{product[index].id}</span>
                          </div>
                        </td>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-2xl shadow-xl p-6 border-l-4 border-green-500">
            <div className="flex items-center mb-4">
              ...
              <div className="flex-1">
                <h5 className="text-xl font-bold text-gray-800">Manufacture</h5>
          ...
          <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-2xl shadow-xl p-6 border-l-4 border-purple-500">
            <div className="flex items-center mb-4">
              ...
              <div className="flex-1">
                <h5 className="text-xl font-bold text-gray-800">Warehouse</h5>
                <p className="text-sm text-gray-600 mt-1">Only a registered Warehouse can perform this step</p>
              </div>
            </div>
            <form onSubmit={handlerSubmitWarehouse} className="flex gap-3">
              <div className="flex-1 relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
          ...
          <div className="bg-gradient-to-br from-yellow-50 to-orange-100 rounded-2xl shadow-xl p-6 border-l-4 border-orange-500">
            <div className="flex items-start mb-4">
              <div className="w-12 h-12 bg-orange-500 rounded-xl flex items-center justify-center mr-4 shadow-lg flex-shrink-0">
                <svg className="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                </svg>
              </div>
              <div className="flex-1">
                <p className="text-sm text-gray-700 leading-relaxed">
                  Only a registered retailer. Moves the product from warehouse to Retail Stage. Use Step 4 to mark it sold.
                </p>
              </div>
            </div>
          ...
          <div className="bg-gradient-to-br from-red-50 to-pink-100 rounded-2xl shadow-xl p-6 border-l-4 border-red-500">
            <div className="flex items-start mb-4">
              <div className="w-12 h-12 bg-red-500 rounded-xl flex items-center justify-center mr-4 shadow-lg flex-shrink-0">
                <svg className="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
              </div>
              <div className="flex-1">
                <p className="text-sm text-gray-700 leading-relaxed">
                  Only the same retailer who completed Step 3. Enter the product ID and confirm in MetaMask — this sends a second transaction that records Product Sold.
                </p>
              </div>
            </div>
